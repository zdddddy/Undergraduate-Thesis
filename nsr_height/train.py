...
def warp_prev_to_current(prev_map, prev_pose7, cur_pose7, map_size, prev_valid=None, gravity_aligned=True):
    ...
    # 重力对齐模式只使用 XY+Yaw；全 6DoF 下仍是 2.5D 近似。
...
def build_model_input(meas_h, meas_m, prev_h, prev_valid, in_channels):
    if in_channels == 2:
        return torch.cat([meas_h, meas_m], dim=1)
    if in_channels == 4:
        return torch.cat([meas_h, meas_m, prev_h, prev_valid], dim=1)
    if in_channels == 3:
        return torch.cat([meas_h, meas_m, prev_h], dim=1)
    raise ValueError(f'Unsupported in_channels={in_channels}; expected 2, 3, or 4.')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_dir', type=str, default=os.path.join(RESULTS_ROOT, 'nsr', 'logs', 'height_experiment_1'))
    parser.add_argument('--checkpoint_dir', type=str, default=os.path.join(RESULTS_ROOT, 'nsr', 'checkpoints'))
    ...
    parser.add_argument('--in_channels', type=int, default=4)  # 当前高度、当前mask、上一帧预测、上一帧mask
